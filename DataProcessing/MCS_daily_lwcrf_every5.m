...
for date = startdate:enddate
    ...
    for hr = 1:2:48
        ...
        for j = 20:20:180
            for i = 0:20:1420
                if i == 0
                    [val,lon_c]=min(abs(lon-lon_ori(1440)));
                else
                    [val,lon_c]=min(abs(lon-lon_ori(round(i))));
                end
                [val,lat_c]=min(abs(lat-lat_ori(round(j))));
    ...
    save(['/neelin2020/ACRE/Processed/MCS_std_ceres_lwcrf_every5_10x10rc_' yyyy mm dd '_new_ave.mat'],'cell_lwcrf');
    clear cell_lwcrf;
end
end
end
