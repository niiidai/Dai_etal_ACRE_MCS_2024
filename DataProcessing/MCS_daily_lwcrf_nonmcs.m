...
for date = startdate:enddate
    ...
    for hr = 1:2:48
        ir_mcs_mask_new = reshape(mean(reshape(mean(reshape(squeeze(ir_mcs_mask(:,:,hr)),xdim_ori,blk(2),[]),2),blk(1),[]),1),[xdim,ydim])>=0.9375;
        ir_nomcs_mask_new = reshape(mean(reshape(mean(reshape(squeeze(ir_nomcs_mask(:,:,hr)),xdim_ori,blk(2),[]),2),blk(1),[]),1),[xdim,ydim])>=0.9375;
	fn_pr = ['/neelin2020/ACRE/IMERG/3B-HHR.MS.MRG.3IMERG.' yyyy mm dd '-' IMERG_hr((hr+1)/2,:) '.V06B.HDF5_re025.nc'];
        pr_ori = ncread(fn_pr,'precipitationCal');
        pr_ori(pr_ori<-1000.) = NaN;
        pr_ori = permute(pr_ori,[2 1]);
        mask_pr = pr_ori>0;
        tmp = logical(ir_nomcs_mask(:,:,hr).*mask_pr);
        CC = bwconncomp(tmp);
        s = regionprops(CC, {'area','centroid'});
        cents = zeros(numel(s),2);
        cents_area = zeros(numel(s),1);

	fn_pr = ['/neelin2020/ACRE/IMERG/3B-HHR.MS.MRG.3IMERG.' yyyy mm dd '-' IMERG_hr((hr+1)/2,:) '.V06B.HDF5_re1.nc'];
        ...
        for i = 1:numel(s)
            ...
            pr_tmp(pr_tmp<=0) = NaN;
	    m_ceres_all = mean(all_lw_crf(:),'omitnan');
            ...
            cell_lwcrf(end+1,:) = {m_ceres_all,m_ceres_all_mcs,m_ceres_all_nonmcs,m_ceres_all_nonmcs_pr2,m_ceres_all_nonmcs_dry2,m_ceres_all_mcs_pr2,m_ceres_all_mcs_dry2,hr};
        end
    end
    save(['/neelin2020/ACRE/Processed/MCS_std_ceres_lwcrf_10x10nonrc_' yyyy mm dd '_new_ave.mat'],'cell_lwcrf');
    clear cell_lwcrf;
end
end
end
