...
for date = startdate:enddate
    ...
    for hr = 1:2:48
        ...
        for i = 1:numel(s)
            ...
            %tmp = all_lw_crf(logical(ir_nomcs_mask_tmp.*mask_pr));
            %m_ceres_all_nonmcs_pr0 = mean(tmp(:),'omitnan');
            %tmp = all_lw_crf(logical(ir_nomcs_mask_tmp.*mask_dry));
            %m_ceres_all_nonmcs_dry0 = mean(tmp(:),'omitnan');
            mask_pr = pr_tmp>2; mask_dry = pr_tmp<=2;
            ...
            %mask_pr = pr_tmp>0 & pr_tmp<=2; mask_dry = pr_tmp==0;
            %tmp = all_lw_crf(logical(ir_mcs_mask_tmp.*mask_pr));
            %m_ceres_all_mcs_pr0 = mean(tmp(:),'omitnan');
            %tmp = all_lw_crf(logical(ir_mcs_mask_tmp.*mask_dry));
            %m_ceres_all_mcs_dry0 = mean(tmp(:),'omitnan');
            %cell_lwcrf(end+1,:) = {m_ceres_all,m_ceres_all_mcs,m_ceres_all_nonmcs,m_ceres_all_nonmcs_pr2,m_ceres_all_nonmcs_dry2,m_ceres_all_mcs_pr2,m_ceres_all_mcs_dry2,m_ceres_all_nonmcs_pr0,m_ceres_all_nonmcs_dry0,m_ceres_all_mcs_pr0,m_ceres_all_mcs_dry0,hr};
    ...
    save(['/neelin2020/ACRE/Processed_ACRE_revised/MCS_std_ceres_lwcrf_10x10rc_' yyyy mm dd '_new_ave.mat'],'cell_lwcrf');
    clear cell_lwcrf;
end
end
end
