...
for date = startdate:enddate
    ...
    for hr = 1:2:48
        ...
        for i = 1:numel(s)
            ...
            elseif lat_c-mo>=1 && lat_c+mo<=ydim && lon_c-mo<1
                ...
                ir_mcs_mask_tmp = [squeeze(ir_mcs_mask_nosmall(lon_c1:xdim_ori,lat_c1:lat_c2));squeeze(ir_mcs_mask_nosmall(1:lon_c2,lat_c1:lat_c2))];
                ir_nomcs_mask_tmp = [squeeze(ir_nomcs_mask_nosmall(lon_c1:xdim_ori,lat_c1:lat_c2));squeeze(ir_nomcs_mask_nosmall(1:lon_c2,lat_c1:lat_c2))];
            ...
            elseif lat_c-mo>=1 && lat_c+mo<=ydim && lon_c+mo>xdim
                ...
                [val,lat_c1]=min(abs(lat_ori-lat1));
                [val,lat_c2]=min(abs(lat_ori-lat2));
                ir_mcs_mask_tmp = squeeze(ir_mcs_mask_nosmall(lon_c1:lon_c2,lat_c1:lat_c2));
                ir_nomcs_mask_tmp = squeeze(ir_nomcs_mask_nosmall(lon_c1:lon_c2,lat_c1:lat_c2));
    ...
    save(['/neelin2020/ACRE/Processed_ACRE_revised/MCS_std_ceres_vv_10x10rc_' yyyy mm dd '_new_ave.mat'],'cell_vv');
    clear cell_vv;
end
end
end
